...
void SemAllocHandler(int passes){
  ...
  MyBzero((char *)&sem[sid], sizeof(sem_t));
...
void PortHandler(void){
  ...
  for(port_num=0; port_num<PORT_NUM-1; port_num++){ // PORT_NUM equals 3 (COM Ports 2, 3 4)
  ...
  outportb(0x20, 0x63); // 0x23 dismiss IRQ3 
  outportb(0x20, 0x64); // 0x24 dismiss IRQ4
  //outportb(0x20, ~(1+8+16));
}
...
void PortAllocHandler(int *eax){
  ...
  *eax = port_num;
  ...
  MyBzero((char *)&port[port_num].IO+DATA, sizeof(port_t));//sizeof(port_t)*DATA);
  ...
  outportb(port[port_num].IO+CFCR, CFCR_7BITS|CFCR_PEVEN|CFCR_PENAB);
  //CFCR_PEVEN|CFCR_PENAB|CFCR_7BITS);

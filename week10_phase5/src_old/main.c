...
void Scheduler(){ // choose a PID as current_pid to load/run
   if(current_pid == 1) return; // if continue below, find one for current_pid
...
int main() {
   ...
   MyBzero((char *)&free_q, (sizeof(q_t))*Q_SIZE);
   MyBzero((char *)&ready_q, (sizeof(q_t))*Q_SIZE);
   MyBzero((char *)&sleep_q, (sizeof(q_t))*Q_SIZE);
  ...
  for(i=1; i<Q_SIZE; i++){
   ...
   outportb(0x21, ~(1+8+16)); // set PIC mask to open up for IRQ0, IRQ1, IRQ3 and IRQ4 
   //outportb(0x21, 0x63);
   //outportb(0x21, 0x64);
